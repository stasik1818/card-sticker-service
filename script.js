  ...
  reader.onload = function (event) {
    try {
      ...
      imagePreview.onload = function() {
        ...
        let initialScale = Math.max(frameWidth / imageWidth, frameHeight / imageHeight) * 1.2;
...
async function submitImage() {
  ...
  const scaleX = imageWidth / frameRect.width;
  const scaleY = imageHeight / frameRect.height;
  const sx = (-translateX * scaleX) / scale;
  const sy = (-translateY * scaleY) / scale;
  const sWidth = (frameRect.width * scaleX) / scale;
  const sHeight = (frameRect.height * scaleY) / scale;
